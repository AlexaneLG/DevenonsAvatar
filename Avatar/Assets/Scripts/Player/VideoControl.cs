...
public class VideoControl : MonoBehaviour
{
	...
	private void LoadFileToMemory(string folder, string filename)
	{
		...
		if (File.Exists(filePath))
		{
			...
			if (bytes.Length > 0)
			{
				...
				_movie.LoadMovieFromMemory(true, filename, _moviePtr, _movieLength, FilterMode.Trilinear, TextureWrapMode.Clamp);
			}
		}
	}
    ...
    public void GUIManager()
    {
        ...
        videoSlider.maxValue = moviePlayer.DurationSeconds;

        if (usingSlider)
            moviePlayer.PositionSeconds = videoSlider.value;
        else
            videoSlider.value = moviePlayer.PositionSeconds;
