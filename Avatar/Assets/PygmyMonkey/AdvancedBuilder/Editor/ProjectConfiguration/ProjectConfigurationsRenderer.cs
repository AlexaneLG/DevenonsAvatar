			...
			if (GUIUtils.DrawHeader("Configuration information", true))
			{
				...
				GUILayout.BeginHorizontal();
				{
					GUILayout.BeginVertical();
					{
						...
						else if (configuration.platformType == PlatformType.Linux)
						{
							configuration.isHeadlessMode = EditorGUILayout.ToggleLeft(new GUIContent("Headless mode", "Build headless Linux standalone."), configuration.isHeadlessMode);
						}
						...
						GUILayout.BeginHorizontal();
						{
							...
							{
								configuration.applyConfiguration(false);
							}
