...
namespace SWS
{
    ...
    public class PathManager : MonoBehaviour
    {
        ...
        public void CreateAndCenter(Transform parent = null)
        {
            if (parent == null)
                parent = transform;
            ...
            Create(childs.ToArray(), false, true);
        }
        ...
        /// </summary>
        public virtual void Create(Transform[] waypoints, bool makeChildren = false, bool center = false)
        {
            ...
            if (center)
            {
                ...
                for (int i = 0; i < waypoints.Length; i++)
                {
                    bary += waypoints[i].position;
                }
                bary /= waypoints.Length;

                Vector3 delta = transform.position - bary;
                transform.position = bary;

                for (int i = 0; i < waypoints.Length; i++)
                {
                    waypoints[i].position = waypoints[i].position + delta;
                }
            }
